...
void check_pause() {
    if (digitalRead(PauseButton) == HIGH) { // see if active High, originally worked like this for test
        isPaused = true; 
        Serial.println("Is paused."); 
    
    while (isPaused) {
      if (digitalRead(START_BUTTON) == HIGH) {
        delay(50);  // Debounce if user slightly touches the button or electrical noise. Waits 50 miliseconds for determined start. 
        if (digitalRead(START_BUTTON) == HIGH) {
          isPaused = false;
      ...
      delay(10); // small delay between whiles. 
    }
  }
...
void setup() {
  ...
  pinMode(PauseButton, INPUT); // Get the pause button as input 
  pinMode(Startbutton, INPUT); // interrupt for start 

  axis1.attach(6);
  axis2.attach(9);
  axis1.write(90); // write to original position (when both arms are at 90 degrees)
  axis2.write(90);
  delay(500);
